...
public class MicVolumeSample : MonoBehaviour {
    ...
    public void Initialize () {
        ...
        if ((aud != null)&&(Microphone.devices.Length>0)) // オーディオソースとマイクがある
        {
            ...
            foreach(var dev in Microphone.devices){
                Debug.Log("device name : + "  + dev);
                DeviceList.text += dev + "¥n";

            }
            string devName = Microphone.devices[0]; // 複数見つかってもとりあえず0番目のマイクを使用
            int minFreq, maxFreq;
            Microphone.GetDeviceCaps("", out minFreq, out maxFreq); // 最大最小サンプリング数を得る
            Debug.Log("freq : " + minFreq + " " + maxFreq);
            aud.clip = Microphone.Start(devName, true, 2, 44100); // iOSではGetDeciceCapsで0しか返ってこないので適当なサンプリングレート
    ...
    void Update () {
        ...
        indicator.text = m_volumeRate.ToString();
    }
